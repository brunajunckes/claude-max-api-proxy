...
/**
 * Each entry: { id, family, alias, timeoutMs }
...
const MODEL_DEFINITIONS = [
    ...
    { id: "claude-opus-4-6", family: "opus",   alias: "opus",   timeoutMs: 1800000 },
    { id: "claude-opus-4",   family: "opus",   alias: "opus",   timeoutMs: 1800000 },
    { id: "claude-opus-4-5", family: "opus",   alias: "opus",   timeoutMs: 1800000 },
    // Sonnet — current model first
    { id: "claude-sonnet-4-6", family: "sonnet", alias: "sonnet", timeoutMs: 600000 },
    { id: "claude-sonnet-4",   family: "sonnet", alias: "sonnet", timeoutMs: 600000 },
    { id: "claude-sonnet-4-5", family: "sonnet", alias: "sonnet", timeoutMs: 600000 },
    // Haiku — current model first
    { id: "claude-haiku-4-5", family: "haiku", alias: "haiku", timeoutMs: 120000 },
    { id: "claude-haiku-4",   family: "haiku", alias: "haiku", timeoutMs: 120000 },
];

// Provider prefixes that clients may prepend
const PROVIDER_PREFIXES = ["maxproxy/", "claude-code-cli/"];

// Build lookup map: model string -> { alias, timeoutMs }
// Includes bare ids, prefixed ids, and bare family aliases
const MODEL_LOOKUP = new Map();

for (const def of MODEL_DEFINITIONS) {
    const entry = { alias: def.alias, timeoutMs: def.timeoutMs };
...
for (const family of ["opus", "sonnet", "haiku"]) {
    ...
    if (def) {
        MODEL_LOOKUP.set(family, { alias: def.alias, timeoutMs: def.timeoutMs });
    }
}
 ...
 * Falls back to 180s (safe middle ground) for unknown models.
 */
...
// Uses the first entry per family from MODEL_DEFINITIONS.
const CANONICAL_IDS = {};
for (const def of MODEL_DEFINITIONS) {
 ...
 *
 * Used when converting CLI output back to OpenAI response format.
 */
