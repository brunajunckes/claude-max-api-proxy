...
describe('CacheManager', () => {
    ...
    test('concurrent set operations maintain cache integrity', async () => {
        ...
        const promises = Array.from({ length: 50 }, (_, i) => Promise.resolve().then(() => {
    ...
    test('concurrent get/set operations with TTL expiry', (t, done) => {
        ...
        setTimeout(() => {
            ...
            assert.equal(statsAfter.misses > 0, true);
            done?.();
        }, 150);
    });
    ...
    test('memory pressure with many small entries', () => {
        ...
        assert.equal(stats.evictions, 30);
    });
});
...
//# sourceMappingURL=cache-middleware.test.js.map