...
describe('CircuitBreaker - Exponential Backoff', () => {
  test('backoff increases with failures in HALF_OPEN', async () => {
    ...
    // Force open and get initial backoff
    try {
      await breaker.execute(() => Promise.reject(new Error('fail')));
    } catch {
      // Expected
    ...
    // Get metrics just before testing in HALF_OPEN
    const state1 = breaker.getState();

    if (state1 === CircuitBreakerState.HALF_OPEN) {
      const metricsBeforeHalfOpenFail = breaker.getMetrics();
      const multiplierBeforeFail = metricsBeforeHalfOpenFail.backoffMultiplier;

      // Fail in HALF_OPEN - this increases the multiplier
      try {
        await breaker.execute(() => Promise.reject(new Error('fail')));
      } catch {
        // Expected
      ...
      // After failure in HALF_OPEN, multiplier should increase
      assert.ok(metricsAfterHalfOpenFail.backoffMultiplier > multiplierBeforeFail);
    }
  });
...
describe('CircuitBreaker - Sync Operations', () => {
  ...
  test('executeSync throws when circuit is OPEN', async () => {
    ...
    assert.throws(
      () => breaker.executeSync(() => 'should fail'),
      CircuitBreakerOpenError
    );
  });
...
describe('CircuitBreaker - Generic Types', () => {
  ...
  test('sync version with generics', () => {
    ...
    const breaker = new CircuitBreaker<NumberResult>();
