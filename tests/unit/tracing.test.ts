...
describe('TracingManager', () => {
  ...
  test('getAllActiveSpans returns array of contexts', () => {
    ...
    assert.ok(activeSpans.every((span: any) => span.traceId));
...
describe('TracingManager - Concurrent Operations', () => {
  test('concurrent span creation for different traces', async () => {
...
describe('TracingManager - Advanced Context Propagation', () => {
  test('W3C traceparent header with all fields', () => {
  ...
  test('tracestate header is preserved', () => {
    const tracing = new TracingManager();
    const context: TraceContext = {
      traceId: 'trace-123',
      spanId: 'span-456',
    ...
    const injected = tracing.injectTraceContext(context);
    assert.ok(injected['tracestate']);
    assert.ok(typeof injected['tracestate'] === 'string');
  });

  test('multiple child spans from same parent', () => {
    const tracing = new TracingManager();
    ...
    const children = [];
    for (let i = 0; i < 5; i++) {
      children.push(tracing.createChildSpan(parent));
    }
  ...
  test('span context maintains parent chain', () => {
    const tracing = new TracingManager();
    const grandparent: TraceContext = {
      traceId: 'trace-gp',
      spanId: 'span-gp',
      sampled: true
    };

    const parent = tracing.createChildSpan(grandparent);
    const child = tracing.createChildSpan(parent);

    assert.equal(child.traceId, 'trace-gp');
    assert.equal(child.parentSpanId, parent.spanId);
  });

  test('span events contain operation metadata', () => {
    const tracing = new TracingManager();
    const traceId = 'trace-metadata-' + Math.random().toString();

    tracing.recordOperationSpan(traceId, 'http.request', 45, {
      method: 'POST',
      path: '/api/test',
      statusCode: 200
    });
    ...
    assert.equal(lastEvent.name, 'http.request');
    assert.ok(lastEvent.attributes);
    assert.equal(lastEvent.attributes!.method, 'POST');
    assert.equal(lastEvent.attributes!.statusCode, 200);
  });

  test('tracing handles rapid span creation', () => {
    const tracing = new TracingManager();
    const context: TraceContext = {
      traceId: 'rapid-trace',
      spanId: 'rapid-span',
    ...
    const ends = [];
    for (let i = 0; i < 100; i++) {
      const end = tracing.startSpan(`rapid-op-${i}`, context);
      ends.push(end);
    }

    const activeCount = tracing.getActiveSpanCount();
    assert.ok(activeCount > 0);

    ends.forEach(end => end());
    const afterCount = tracing.getActiveSpanCount();
    assert.ok(afterCount <= activeCount);
  });

  test('span exporter configuration persists', () => {
    const tracing = new TracingManager();
  ...
  test('multiple flushes clears all events', () => {
    const tracing = new TracingManager();
    const traceId = 'multi-flush-' + Math.random().toString();

    for (let i = 0; i < 5; i++) {
      tracing.recordOperationSpan(traceId, `op-${i}`, i * 10, {});
    }
  ...
  test('span events preserve timing information', () => {
    const tracing = new TracingManager();
    const traceId = 'timing-' + Math.random().toString();
    const duration = 123;

    const before = Date.now();
    tracing.recordOperationSpan(traceId, 'timed-op', duration, {});
    const after = Date.now();

    const events = tracing.getSpanEvents(traceId);
    const event = events[events.length - 1];

    // The startTime should be approximately 'before - duration'
    // Allow some tolerance for execution time
    const expectedStart = before - duration;
    assert.ok(event.startTime >= expectedStart - 50, `startTime ${event.startTime} should be ~${expectedStart}`);
    assert.ok(event.startTime <= before + 50, `startTime ${event.startTime} should be before or near ${before}`);
  });

  test('error status propagation in spans', () => {
    const tracing = new TracingManager();
    const traceId = 'error-trace-' + Math.random().toString();

    tracing.recordOperationSpan(traceId, 'failed-op', 100, {
      error: true,
      errorCode: 'TIMEOUT',
  ...
  test('span context with various trace formats', () => {
    const tracing = new TracingManager();

    const formats = [
      { traceparent: '00-0af7651916cd43dd-b7ad6b7169203331-00' },
      { traceparent: '00-12345678901234567890123456789012-1234567890123456-01' },
      { 'X-Trace-Id': 'custom-trace-id' }
    ];

    formats.forEach(headers => {
      const context = tracing.extractTraceContext(headers);
      assert.ok(context.traceId);
    });
  });

  test('getAllActiveSpans returns correct metadata', () => {
    const tracing = new TracingManager();
    const context: TraceContext = {
      traceId: 'all-spans-trace',
      spanId: 'all-spans-span',
      sampled: true
    };

    const end1 = tracing.startSpan('op1', context);
    const end2 = tracing.startSpan('op2', context);

    const activeSpans = tracing.getAllActiveSpans();

    assert.ok(Array.isArray(activeSpans));
    assert.ok(activeSpans.length >= 2);
    activeSpans.forEach((span: any) => {
  ...
  test('span with null attributes handled gracefully', () => {
    const tracing = new TracingManager();
    const traceId = 'null-attrs-' + Math.random().toString();

    tracing.recordOperationSpan(traceId, 'op-with-null', 50, null as any);

    const events = tracing.getSpanEvents(traceId);
    const event = events[events.length - 1];

    assert.ok(event);
    assert.equal(event.name, 'op-with-null');
  });

  test('concurrent tracing across multiple managers', async () => {
    const managers = [
      new TracingManager(),
      new TracingManager(),
      new TracingManager()
    ...
    const promises = managers.map((mgr, idx) => {
      return Promise.resolve().then(() => {
        const context: TraceContext = {
          traceId: `concurrent-${idx}`,
          spanId: `span-${idx}`,
          sampled: true
        };

        mgr.recordOperationSpan(`concurrent-${idx}`, `op-${idx}`, idx * 10, {});
        return mgr.getSpanEvents(`concurrent-${idx}`);
      });
    });

    const results = await Promise.all(promises);
    assert.equal(results.length, 3);
    results.forEach(result => assert.equal(result.length, 1));
  });

  test('span injection and extraction roundtrip', () => {
    const tracing = new TracingManager();
    const original: TraceContext = {
      // Use valid W3C Trace Context format (32 hex chars for traceId)
      traceId: '4bf92f3577b34da6a3ce929d0e0e4736',
      // 16 hex chars for spanId
      spanId: '00f067aa0ba902b7',
      sampled: true
    };

    const injected = tracing.injectTraceContext(original);
    const extracted = tracing.extractTraceContext(injected);

    assert.equal(extracted.traceId, original.traceId);
    assert.equal(extracted.spanId, original.spanId);
    assert.equal(extracted.sampled, original.sampled);
  });

  test('span event filtering by status', () => {
    const tracing = new TracingManager();
    const traceId = 'filter-' + Math.random().toString();

    tracing.recordOperationSpan(traceId, 'success-op', 10, {});
    tracing.recordOperationSpan(traceId, 'failed-op', 20, { error: true });
    tracing.recordOperationSpan(traceId, 'success-op2', 30, {});

    const allEvents = tracing.getSpanEvents(traceId);
    assert.equal(allEvents.length, 3);

    const errorEvents = allEvents.filter(e => e.status === 'error');
    assert.equal(errorEvents.length, 1);

    const successEvents = allEvents.filter(e => e.status === 'ok');
